function Get-TLSVersion {
    ...
    $output = [enum]::GetNames([Net.SecurityProtocolType]) | Select @{name='Name'; expression={$_}} , @{name='CurrentSetting'; expression={if($_ -eq [Net.ServicePointManager]::SecurityProtocol){" < "}}}
